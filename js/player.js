...
function startSimpleAutoResumeWatcher() {
    ...
    _simpleWatcherInterval = registerInterval(setInterval(function () {
        ...
        if (nowOffline && !_simpleWatcherWasOffline) {
            ...
            // Sync network flags so the poller, watchdog, and verified-resume
            // paths all recognize this as a network-paused state.
            _pausedByNetwork = true;
            _lastNetworkErrorTime = Date.now();
            ...
            // Start the dedicated resume poller so retries keep firing
            // even if the simple watcher settle-delay fails.
            try { startPausedByNetworkResumePoller(); } catch (ePoll) {}
            return;
        }
...
function showPlayerErrorPopup(title, message) {
    ...
    if (popup) {
        ...
        if (playerErrorPopupOpen && playerLastErrorCategory && playerLastErrorCategory === incomingCategory) {
            ...
            if (incomingCategory === 'subscription') {
                ...
                if (existingPopupChName) {
                    ...
                    var existingChDecoded = (typeof decodeHtmlEntities === 'function') ? decodeHtmlEntities(existingCh) : existingCh;
                    existingPopupChName.textContent = existingChDecoded || '';
                    existingPopupChName.style.display = existingChDecoded ? '' : 'none';
                }
        ...
        if (popupChName) {
            ...
            var chDecoded = (typeof decodeHtmlEntities === 'function') ? decodeHtmlEntities(ch) : ch;
            popupChName.textContent = chDecoded || '';
            popupChName.style.display = chDecoded ? '' : 'none';
        }
...
function initializePlayerApp() {
    ...
    consumeFirstLaunchSidebarFlowFlag();
    console.log('[PLAYER] Player initialization started');

    // Hide loading overlay immediately so key events are never blocked.
    hidePageLoadingOverlay();
...
var _forceFirstLaunchSidebarFlow = false;
var isMenuOpen = false;
var hasMenuOpenedSinceLaunch = false;

function setMenuOpenState(open) {
    isMenuOpen = !!open;
    if (isMenuOpen) {
        hasMenuOpenedSinceLaunch = true;
    }
    if (typeof sidebarState !== 'undefined' && sidebarState) {
        sidebarState.isOpen = isMenuOpen;
    }
}

function resetClosedMenubarNumberOrder() {
    var closedSource = (_allChannelsUnfiltered && _allChannelsUnfiltered.length > 0)
        ? _allChannelsUnfiltered
        : allChannels;
    applyDefaultClosedMenubarOrdering(closedSource, { resetToTop: true });
    try {
        var channelsListEl = document.getElementById('channelsList');
        var sidebarScrollEl = document.querySelector('#playerSidebar .sidebar-scroll-content');
        if (channelsListEl) channelsListEl.scrollTop = 0;
        if (sidebarScrollEl) sidebarScrollEl.scrollTop = 0;
    } catch (e) {}
}

function consumeFirstLaunchSidebarFlowFlag() {
    try {
        if (sessionStorage.getItem('bbnl_player_use_first_launch_sidebar') === '1') {
            _forceFirstLaunchSidebarFlow = true;
            sessionStorage.removeItem('bbnl_player_use_first_launch_sidebar');
...
function applyDefaultClosedMenubarOrdering(sourceChannels, options) {
    options = options || {};
    ...
    // Launch-time closed-menubar behavior: Ch 999 first, then subscribed
    // channels in ascending channel-number order.
    var subscribedOnly = base.filter(function (ch) {
        return isChannelSubscribed(ch);
    });
    var fofiChannels = base.filter(function (ch) {
        return isFoFiChannel(ch);
    });
    var numericSource = (subscribedOnly.length > 0 ? subscribedOnly : base).filter(function (ch) {
        return !isFoFiChannel(ch);
    });
    var ordered = fofiChannels.concat(numericSource);
    ordered.sort(function (a, b) {
        var aFoFi = isFoFiChannel(a);
        var bFoFi = isFoFiChannel(b);
        if (aFoFi !== bFoFi) return aFoFi ? -1 : 1;
...
async function loadChannelList(lookupName = null) {
    try {
        ...
        if (Array.isArray(allResponse) && allResponse.length > 0) {
            ...
            // Before the first menu open, closed playback uses Ch 999 + numeric
            // launch flow. After the menu has opened once, keep menu context.
            if ((!sidebarState || !sidebarState.isOpen) && !hasMenuOpenedSinceLaunch) {
                ...
                setTimeout(async function () {
                    try {
                        ...
                        if (freshData && freshData.length > 0) {
                            ...
                            if ((!sidebarState || !sidebarState.isOpen) && !hasMenuOpenedSinceLaunch) {
                                applyDefaultClosedMenubarOrdering(sortedFresh);
                            } else {
                                // Re-apply current menu context once the user has opened
                                // the menubar, even if it is currently closed.
...
function getChannelInitials(channel) {
    var name = '';
    if (channel) {
        var rawName = String(channel.chtitle || channel.channel_name || '').trim();
        name = (typeof decodeHtmlEntities === 'function') ? decodeHtmlEntities(rawName) : rawName;
    }
    if (!name) return '?';
...
function updatePlayerChannelLogo(channel) {
    ...
    var channelAltText = channel.chtitle || channel.channel_name || '';
    var channelAltDecoded = (typeof decodeHtmlEntities === 'function') ? decodeHtmlEntities(channelAltText) : channelAltText;
    newImg.alt = channelAltDecoded;
...
function setupPlayer(channel) {
    ...
    var chNameRaw = channel.channel_name || channel.chtitle || "Unknown Channel";
    var chName = (typeof decodeHtmlEntities === 'function') ? decodeHtmlEntities(chNameRaw) : chNameRaw;
    ...
    if (latestSubscribed === "no" || latestSubscribed === "No" || latestSubscribed === "NO" ||
        ...
        if (isFromNumberSearch) {
            // For number search: keep the current language when it still matches,
            // otherwise redirect by the same rules used after playback.
            try {
                if (sidebarState && sidebarState.languages && sidebarState.languages.length > 0) {
                    var targetIdx = resolveSidebarLanguageIndexForEnteredChannel(latestKnownChannel);
                    ...
                    if (typeof applySidebarLanguageToZapListAndSession === 'function') {
                        applySidebarLanguageToZapListAndSession(latestKnownChannel);
                    }
...
function changeChannel(step) {
    ...
    // ✅ CRITICAL FIX: Remote CH+/- navigation behavior
    // When SIDEBAR IS OPEN: Use the selected category/language from sidebar
    // When SIDEBAR IS CLOSED: Use number sequence (always!) - menu is invisible so user shouldn't see category changes
    if (isMenuOpen) {
        // Sidebar is open - respect the user's visible category/language selection
        activeCategoryIndex = sidebarState.categoryIndex;
        // activeLangIndex stays as sidebarState.languageIndex (already set above)
    } else if (!hasMenuOpenedSinceLaunch) {
        // Launch-time closed mode: use Ch 999 + numeric channel sequence.
        activeLangIndex = -1;
        ...
        try {
            var closedSource = (_allChannelsUnfiltered && _allChannelsUnfiltered.length > 0)
                ? _allChannelsUnfiltered
                : allChannels;
            applyDefaultClosedMenubarOrdering(closedSource);
        } catch (eClosedOrder) {}
    } else {
        // After the menu has opened once, keep its selected category/language
        // context even when the menu is closed.
    ...
    if (sidebarState && Array.isArray(sidebarState.languages) && activeLangIndex >= 0) {
        ...
        // once the menubar flow has been entered.
        else if (langCode !== 'all' && (isMenuOpen || hasMenuOpenedSinceLaunch)) {
            useFilteredNavigation = true;
        }
        // All Channels should follow the visible sidebar order, not raw numeric order.
    ...
    if (useFilteredNavigation) {
        ...
        // FoFi/default launch is outside the normal list on some backends.
        // Before the menubar is opened, CH+ must start at LCN 1, not skip to 2.
        if (!hasMenuOpenedSinceLaunch && step > 0 && currentIndex < 0) {
            nextIndex = 0;
        }
...
function getNextChannelInCategory(step, langIndex, categoryIndex) {
    ...
    if (targetLang.code === 'subscribed') {
        ...
        languageChannels = allChannelsForFilter.filter(function(ch) {
            ...
            return chLang === langCode || chLang === langName || chLang.indexOf(langCode) !== -1;
        });
    }
    ...
    // If current channel not found, start from the beginning/end of the active
    // category sequence depending on direction.
    if (currentSequenceIndex === -1) {
        currentSequenceIndex = (step > 0) ? -1 : orderedChannels.length;
    }

    // Calculate next index inside the selected category only.
    var nextCategoryIndex = currentSequenceIndex + step;
    
    // Wrap around inside the selected category.
    if (nextCategoryIndex >= orderedChannels.length) nextCategoryIndex = 0;
    if (nextCategoryIndex < 0) nextCategoryIndex = orderedChannels.length - 1;

    // Update global currentIndex to match the selected channel in the main allChannels list
    var nextChannel = orderedChannels[nextCategoryIndex];
    if (nextChannel) {
...
function getNextChannelInLanguage(step, activeLangIndex) {
    ...
    if (targetLang.code === 'subscribed') {
        ...
        // All Channels: follow the same visible order used by the sidebar.
        languageChannels = (Array.isArray(sidebarState.channels) && sidebarState.channels.length > 0)
            ? sidebarState.channels.slice()
            : allChannelsForFilter.slice();
        applySidebarChannelSortToList(languageChannels);
    } else {
...
function saveCurrentLanguageUiState() {
    ...
    // Save currently expanded categories by NAME so they survive
    // API-driven category reordering when switching tabs (◄ ►).
    // The playing channel's category is always re-expanded on open,
    // so this primarily preserves user-initiated expansions during
    // tab switching within the same sidebar session.
    var expandedNames = [];
    var expandedName = '';
    var expIndices = getSortedExpandedCategoryIndices();
    for (var ei = 0; ei < expIndices.length; ei++) {
        var eCat = sidebarState.categories[expIndices[ei]];
        if (eCat && eCat.name) {
            expandedNames.push(String(eCat.name));
            if (!expandedName) expandedName = String(eCat.name);
        }
    }
...
/**
 * Wipe expansion history for all language tabs.
 * Call this when the sidebar is CLOSED to ensure history doesn't survive.
 */
function clearAllLanguageUiExpansionHistory() {
    if (!sidebarState.languageUiState) return;
    Object.keys(sidebarState.languageUiState).forEach(function (key) {
        var state = sidebarState.languageUiState[key];
        if (state) {
            state.expandedCategoryNames = [];
            state.expandedCategoryName = '';
        }
    });
}
...
function applyPreferredSidebarLanguage() {
    ...
    try {
        if (!_forceFirstLaunchSidebarFlow) {
    ...
    if (!preferredLangId && !preferredLangName) {
        _hasExplicitPreferredLanguage = false;
        ...
        try {
            if (typeof OperatorDefaults !== 'undefined' && OperatorDefaults.getLanguageIndexForCurrentUser) {
                ...
                // Fallback: default to "All Channels" (index 0)
                sidebarState.languageIndex = 0;
            }
        } catch (eOpDefault) {
            sidebarState.languageIndex = 0; // Fallback to "All Channels"
        }
        updateLanguageDisplay();
        return;
    }

    _hasExplicitPreferredLanguage = true;
...
async function initializeSidebar() {
    ...
    // Load available channels (WAIT for hydration)
    await loadSidebarChannels();

    // Keep CH+/CH- source list aligned with the resolved default language
    // before any menu interaction (critical for operator default = All Channels).
    try {
        applySidebarLanguageToZapListAndSession(_lastAttemptedChannel || _lastPlayingChannel || null);
    } catch (eZapInitSync) {}

    // Fix Issue: Sync sidebar with current playing channel immediately on launch
    // Now that loadSidebarChannels is awaited, this will have valid data.
    alignSidebarToCurrentPlayback();
    saveCurrentLanguageUiState();
}
...
function changeLanguage(direction) {
    ...
    // Auto-expand the category containing the currently playing channel so it
    // remains visible and focused when the user switches tabs (◄ ►). Without
    // this, switching to another tab and back would leave all categories collapsed.
    try {
        var playingCatIdx = getCurrentPlayingCategoryIndex();
        if (playingCatIdx >= 0 && playingCatIdx < sidebarState.categories.length) {
            if (!isSidebarCategoryExpanded(playingCatIdx)) {
                setSidebarCategoryExpanded(playingCatIdx, true);
                renderCategoriesList();
            }
            // Load channels for the playing channel's category and focus
            sidebarState.channels = getChannelsForCategoryAtIndex(playingCatIdx) || [];
            renderChannelsList();
            var playingChIdx = findCurrentChannelInSidebar();
            if (playingChIdx >= 0) {
                sidebarState.categoryIndex = playingCatIdx;
                sidebarState.channelIndex = playingChIdx;
                sidebarState.currentLevel = 'channels';
                setTimeout(function () {
                    if (sidebarState.isOpen) {
...
function findSidebarLanguageIndexByCode(targetCode) {
    ...
    for (var i = 0; i < sidebarState.languages.length; i++) {
        var lang = sidebarState.languages[i] || {};
        var langCode = String(lang.code || '').trim().toLowerCase();
        var langName = String(lang.name || '').trim().toLowerCase();
        if (langCode === normalizedTarget || langName === normalizedTarget) return i;
    }
    return -1;
}

function channelMatchesSidebarLanguage(channel, langIndex) {
    if (!channel || !Array.isArray(sidebarState.languages) || langIndex < 0 || langIndex >= sidebarState.languages.length) {
        return false;
    }

    var lang = sidebarState.languages[langIndex] || {};
    var langCode = String(lang.code || '').trim().toLowerCase();
    if (langCode === 'all') return true;
    if (langCode === 'subscribed') return isChannelSubscribed(channel);

    var channelLangId = String(channel.langid || channel.lang_id || '').trim().toLowerCase();
    var channelLangName = String(channel.lalng || channel.langtitle || channel.langname || channel.language || channel.lang || '').trim().toLowerCase();
    var selectedLangId = String(lang.langid || lang.code || '').trim().toLowerCase();
    var selectedLangName = String(lang.name || '').trim().toLowerCase();

    if (channelLangId && selectedLangId && channelLangId === selectedLangId) return true;
    if (channelLangName && selectedLangName && channelLangName === selectedLangName) return true;
    if (channelLangName && selectedLangId && channelLangName === selectedLangId) return true;
    if (channelLangId && selectedLangName && channelLangId === selectedLangName) return true;
    return false;
}

function resolveSidebarLanguageIndexForEnteredChannel(channel) {
    if (!channel || !Array.isArray(sidebarState.languages) || sidebarState.languages.length === 0) return -1;

    var selectedLang = sidebarState.languages[sidebarState.languageIndex] || {};
    var selectedCode = String(selectedLang.code || '').trim().toLowerCase();
    var isSubscribed = isChannelSubscribed(channel);

    if (selectedCode === 'all') {
        return sidebarState.languageIndex;
    }

    if (selectedCode === 'subscribed') {
        return isSubscribed ? sidebarState.languageIndex : findSidebarLanguageIndexByCode('all');
    }

    if (channelMatchesSidebarLanguage(channel, sidebarState.languageIndex)) {
        return sidebarState.languageIndex;
    }

    return findSidebarLanguageIndexByCode(isSubscribed ? 'subscribed' : 'all');
}
...
function getOrderedChannelsForCategoryNavigation(categoryIndex) {
    if (!sidebarState || !Array.isArray(sidebarState.categories) || categoryIndex < 0 || categoryIndex >= sidebarState.categories.length) {
        return [];
    }

    var orderedChannels = [];
    for (var i = categoryIndex; i < sidebarState.categories.length; i++) {
        var forwardChannels = getChannelsForCategoryAtIndex(i);
        if (forwardChannels && forwardChannels.length > 0) {
            orderedChannels = orderedChannels.concat(forwardChannels);
        }
    }
    for (var j = 0; j < categoryIndex; j++) {
        var wrappedChannels = getChannelsForCategoryAtIndex(j);
        if (wrappedChannels && wrappedChannels.length > 0) {
            orderedChannels = orderedChannels.concat(wrappedChannels);
        }
    }
...
function getCurrentPlayingCategoryIndex() {
    ...
    // Check grouped categories first
...
/**
 * Get the language tab index of the currently playing channel.
 */
function getCurrentPlayingLanguageIndex() {
    var current = getCurrentPlayingChannelObject();
    if (!current) return -1;
...
function alignSidebarToCurrentPlayback() {
    ...
    if (_hasExplicitPreferredLanguage) {
        updateLanguageDisplay();
        buildCategoriesForLanguage();
        return;
    }
...
function createChannelItemButton(ch, index, sidebarCategoryIndex) {
    ...
    if (logoUrl && logoUrl.trim() !== '') {
        ...
        var chLogoAlt = ch.chtitle || 'Channel';
        var chLogoAltDecoded = (typeof decodeHtmlEntities === 'function') ? decodeHtmlEntities(chLogoAlt) : chLogoAlt;
        logoImg.alt = chLogoAltDecoded;
    ...
    var chNameText = ch.chtitle || ch.channel_name || 'Unknown';
    var chNameDecoded = (typeof decodeHtmlEntities === 'function') ? decodeHtmlEntities(chNameText) : chNameText;
    nameDiv.textContent = chNameDecoded;
...
function selectCategory(index, preferCurrentChannel) {
    ...
    _lastSelectedLanguageIndex = sidebarState.languageIndex;
    _lastSelectedCategoryIndex = index;
 ...
 * Returns a Promise that resolves when channels are ready.
 */
    ...
    return new Promise(function (resolve) {
        ...
        if (!channelsForSidebar || channelsForSidebar.length === 0) {
            hydrateSidebarAllChannelsCache().then(function () {
                // Always build categories after hydration — even during init
                // when the sidebar isn't open yet. Skipping this when !isOpen
                // was the root cause of missing categories on first launch.
                if (sidebarState) {
                    buildCategoriesForLanguage();
                }
                resolve();
...
function openSidebar() {
    ...
    // NOTE: Do NOT clear expansions here — buildCategoriesForLanguage →
    // restoreCurrentLanguageUiState correctly restores saved expansions.
    // The playing channel's category is also auto-expanded below (line 5825+).
    // A second clearAll here was the root cause of the "subcategory closes
    // when switching back" bug.
...
function closeSidebar() {
    ...
    clearAllSidebarCategoryExpansions();
    clearAllLanguageUiExpansionHistory();
...
function showPlayerUI() {
    ...
    if (sidebar) {
        setMenuOpenState(true);
        try { sessionStorage.setItem('player_menubar_open', '1'); } catch (eMOpenUi) {}
        try { if (window && typeof window.BBNL_setMenubarOpen === 'function') window.BBNL_setMenubarOpen(true); } catch (eNotifyOpenUi) {}
        sidebar.classList.add('open');
        sidebar.classList.remove('close');
    }
...
function hidePlayerUI() {
    ...
    if (sidebar) {
        setMenuOpenState(false);
        try { sessionStorage.removeItem('player_menubar_open'); } catch (eMCloseUi) {}
        try { if (window && typeof window.BBNL_setMenubarOpen === 'function') window.BBNL_setMenubarOpen(false); } catch (eNotifyCloseUi) {}
        sidebar.classList.add('close');
        setTimeout(function () {
            sidebar.classList.remove('open', 'close');
        }, 300);
...
function playChannelByLCNFromPlayer(lcn) {
    ...
    if (channel) {
        ...
        // Switch the menubar category based on the searched channel's
        // language + subscription rules:
        //   - Same language selected    → keep that language
        //   - Different language + sub  → open "Subscribed Channels"
        //   - Different language + unsub→ open "All Channels"
        //   - All Channels selected      → keep "All Channels"
        try {
            if (sidebarState && Array.isArray(sidebarState.languages) && sidebarState.languages.length > 0) {
                var targetIdx = resolveSidebarLanguageIndexForEnteredChannel(channel);
        ...
        showChannelNotFoundToast("Channel Unavailable");
    }
}
...
// Caching frequently-used DOM elements to avoid repeated querySelector/getElementById calls
// ==========================================
var _overlayEl = null;
var _infoBarEl = null;
var _uiChName = null;
var _uiChNum = null;
var _playerSidebarEl = null;
var _playerErrorPopupEl = null;
var _pageLoadingEl = null;
var _playerContainerEl = null;
var _logoBoxEl = null;
var _channelNumberFieldEl = null;
 ...
 */
function getCachedElement(varName, selector) {
